...
export async function initializeChat(apiKey) {
  ...
  const template = ["system",`You are a financial consultant specializing in business solutions for companies looking to optimize their resource allocation based on transaction data. 
...
Francis's transaction data:`];
  ...
  const prompt = ChatPromptTemplate.fromMessages(template, new MessagesPlaceholder("chat_history"),
  ["human", "{question}"],);
...
// Function to send message with context prepended
export async function sendMessage(chain, userQuery, context) {
  const response = await chain.invoke({question: context+ userQuery});
